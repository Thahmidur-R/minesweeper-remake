...
if(window.matchMedia("(max-width:1500px)").matches){
  ...
  if(rows>columns){
    ...
    //let boardRowMax=customMax/rows;
  //boardRowHeight=`${boardRowMax}px`
  ...
  const createTile = (r, c, value) => {
    let content;
if (value === 'bomb') {
  content = <img src="../../public/Minesweeper_bomb.png" className='bombPicture' />;
} else if(value===0){
  content = '';
}
else{content=value}
...
return(
  <div id='gameBoard' className={props.gridClass} style={{display:`${props.display}`, gridTemplateRows:`repeat(${rows}, 1fr)`,gridTemplateColumns:`repeat(${columns}, 1fr)` }}>
    <MakeBoard></MakeBoard>
  </div>
);
